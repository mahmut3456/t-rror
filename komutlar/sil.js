...
if(args[0] > 1300) return message.channel.send(new Discord.MessageEmbed().setDescription(' `100` üzeri mesaj miktarını aynı anda silemem.'));
message.channel.bulkDelete(args[0]);
return message.channel.send(new Discord.MessageEmbed().setDescription(' '+`${args[0]}`+' adet mesaj başarıyla silindi ')).then(m => m.delete({timeout: 2500}));
};
exports.conf = {
  enabled: true,
  guildOnly: true,
